...
def available_total_hotels(driver):
    """
    ...
    """
    try:
        scroll_amount = driver.execute_script("return document.documentElement.scrollHeight") / 2
        ...
        logger.info(f'First scroll: {hotel_count}')

        scroll_amount = driver.execute_script("return document.documentElement.scrollHeight") / 2
        driver.execute_script(f"window.scrollTo({scroll_amount}, {2 * scroll_amount});")
        time.sleep(3)  # Adjust the sleep time if necessary
        ...
        logger.info(f'Total Number of Available Hotels: {hotel_count}')
        return hotel_count
    
    except Exception as e:
        logger.error(f"Error retrieving available hotel count: {str(e)}")
        return 0
